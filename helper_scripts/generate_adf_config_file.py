...
def generate_adf_config(cupid_file, adf_file, out_file):
    ...
    a_dict["diag_cam_baseline_climo"]["cam_case_name"] = c_dict["global_params"][
        "base_case_name"
    ]
    ...
    a_dict["diag_cam_climo"]["cam_hist_loc"] = c_dict["global_params"][
        "CESM_output_dir"
    ]
    a_dict["diag_cam_baseline_climo"]["cam_hist_loc"] = c_dict["global_params"][
        "CESM_output_dir"
    ]

    # QUESTION: how to specify different start/end dates for 'base case' and 'test case(s)'?
    a_dict["diag_cam_climo"]["start_year"] = int(
        c_dict["global_params"]["start_date"].split("-")[0]
    )
    a_dict["diag_cam_climo"]["end_year"] = int(
        c_dict["global_params"]["end_date"].split("-")[0]
    )
    a_dict["diag_cam_baseline_climo"]["start_year"] = int(
        c_dict["global_params"]["start_date"].split("-")[0]
    )
    a_dict["diag_cam_baseline_climo"]["end_year"] = int(
        c_dict["global_params"]["end_date"].split("-")[0]
    )
